...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        LoginDto loginDto = LoginDto.builder().account(account).password(password).verifyCode(verifyCode).build();
        log.info(account + "+++++++++++++" + verifyCode);
        if (redisService.existsKey(account)) {
            log.info("验证码正确");
            //取得redis中的验证码
            String correctCode = redisService.getValue(account, String.class);
